...
public class MainActivity extends AppCompatActivity {
		...
		tmr = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						transfer.setClass(getApplicationContext(), HomeActivity.class);
	...
	private void _CoolAnimationView () {
	...
	private void _startloading () {
		...
		tmr = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						if (check == 0) {
							linear19.setTranslationX((float)(progressnum));
							progressnum++;
							if (progressnum == 1000) {
								check = 1;
							}
		...
		_timer.scheduleAtFixedRate(tmr, (int)(0), (int)(7));
	}
